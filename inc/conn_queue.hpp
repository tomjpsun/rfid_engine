...
public:
	...
	PacketUnit pop() {
		...
		if (size() > 0) {
			...
			LOG(SEVERITY::DEBUG) << LOG_TAG << ", Packet Queue Size 0" << endl;
		}
		return pkt;
	}
	PacketUnit peek(int index) {
		...
		if (size() > 0) {
			...
			LOG(SEVERITY::ERROR) << LOG_TAG << ", Packet Queue Size 0" << endl;
		}
		return pkt;
	}
	...
	void reset() {
		...
		LOG(SEVERITY::DEBUG) << LOG_TAG << "enter async_send" << endl;
		...
		if ( result >= 0) {
			...
			LOG(SEVERITY::ERROR) << LOG_TAG << "ERROR: " << errno << endl;
		}
		LOG(SEVERITY::DEBUG) << LOG_TAG << "leave async_send wait" << endl;
	...
	int send(const std::vector<uint8_t>& cmd) {
		LOG(SEVERITY::DEBUG) << LOG_TAG << "enter sync_send" << endl;
		...
		LOG(SEVERITY::DEBUG) << LOG_TAG << "leave sync_send wait" << endl;
