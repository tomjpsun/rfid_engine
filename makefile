...
LIBS    = -lstdc++ -pthread -lrfid_engine
...
install: $(TARGET_DYN)
	...
	sudo mkdir -p $(PREFIX)/include/rfid_engine
	sudo cp -r inc/* $(PREFIX)/include/rfid_engine
	sudo mkdir -p /etc/rfid_engine
	sudo cp ./rfid_config.json /etc/rfid_engine
