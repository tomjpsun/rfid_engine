...
//         [in] :
//              :
// Return       : If the function is successful, the custom data packet data size
...
int RfidInterface::GeneratePacket(unsigned char * lpbyBuffer, uint64_t uiBufferSize, unsigned int uiPacketType, unsigned char * lpbyOriginal, uint64_t uiOriginalSize, bool fSend)
{
	...
	if (m_fIPv4)
	{
		// Make TCP herader (like)
		PRFID_SNIFFER_PACKET_V4 pstPacketV4 = (PRFID_SNIFFER_PACKET_V4)lpbyBuffer;

		nEtherHeader = sizeof(ETHERNET_HEADER);
		nIpHeader = sizeof(INTERNET_PROTOCOL_VERSION_4);	// 20 Bytes
		nTcpHeader = sizeof(TRANSMISSION_CONTROL_PROTOCOL);	// 20+N Bytes
		nReaderHeader = sizeof(RFID_READER_HEADER);

		nGenerateSize = sizeof(RFID_TCP_V4_HEADER);
		nGenerateSize += uiOriginalSize;

		if (uiBufferSize < nGenerateSize)
			return 0;

		memcpy_s(pstPacketV4->byRaw, _countof(pstPacketV4->byRaw), lpbyOriginal, uiOriginalSize);
		memset(&pstPacketV4->stHeader, 0, sizeof(pstPacketV4->stHeader));
		pstPacketV4->stHeader.stIP.uiVersion = htons(ETHER_TYPE_IPV4);
		pstPacketV4->stHeader.stIP.uiHeaderLength = 0x05; // 20 Bytes
		pstPacketV4->stHeader.stTcp.uiDataOffset = 0x05;  // 20 Bytes
		pstPacketV4->stHeader.stIP.uiTotolLength = htons(nIpHeader + nTcpHeader + nReaderHeader + uiOriginalSize);
		//pstPacketV4->stHeader.stIP.uiTotolLength = htons(20 + 20 + uiOriginalSize);

		if (fSend)
		{
			// IPv4 Address
			pstPacketV4->stHeader.stIP.uiSource = htonl((u_long)m_stLocalAddress.sin_addr.s_addr);
			pstPacketV4->stHeader.stIP.uiDestination = htonl((u_long)m_stHostAddress.sin_addr.s_addr);;

			// TCP Port
			pstPacketV4->stHeader.stTcp.uiSourcePort = htons((u_short)m_stLocalAddress.sin_port);
			pstPacketV4->stHeader.stTcp.uiDestinationPort = htons((u_short)m_stHostAddress.sin_port);
		}
		else
		{
			// IPv4 Address
			pstPacketV4->stHeader.stIP.uiSource = htonl((u_long)m_stHostAddress.sin_addr.s_addr);
			pstPacketV4->stHeader.stIP.uiDestination = htonl((u_long)m_stLocalAddress.sin_addr.s_addr);;

			// TCP Port
			pstPacketV4->stHeader.stTcp.uiSourcePort = htons((u_short)m_stHostAddress.sin_port);
			pstPacketV4->stHeader.stTcp.uiDestinationPort = htons((u_short)m_stLocalAddress.sin_port);
		}

		pstPacketV4->stHeader.stReader.uiType = htonl((u_long)uiPacketType);
		pstPacketV4->stHeader.stReader.uiSerialNo = htonl((u_long)0);
		pstPacketV4->stHeader.stReader.uiLength = htons((u_short)uiOriginalSize);
		//pstPacketV4->stHeader.stReader.ulDeviceID = htonl((u_long)m_ulDeviceID);
		pstPacketV4->stHeader.stReader.uiDeviceID = htonll((u_int64)m_uiDeviceID);
	...
	{
		// Make TCP herader (like)
		PRFID_SNIFFER_PACKET_V6 pstPacketV6 = (PRFID_SNIFFER_PACKET_V6)lpbyBuffer;
		nEtherHeader = sizeof(ETHERNET_HEADER);
		nIpHeader = sizeof(INTERNET_PROTOCOL_VERSION_4);	// 20 Bytes
		nTcpHeader = sizeof(TRANSMISSION_CONTROL_PROTOCOL);	// 20+N Bytes
		nReaderHeader = sizeof(RFID_READER_HEADER);
		nGenerateSize = sizeof(RFID_TCP_V6_HEADER);
		nGenerateSize += uiOriginalSize;
		if (uiBufferSize < nGenerateSize)
			return 0;
		memcpy_s(pstPacketV6->byRaw, _countof(pstPacketV6->byRaw), lpbyOriginal, uiOriginalSize);
		memset(&pstPacketV6->stHeader, 0, sizeof(pstPacketV6->stHeader));
		pstPacketV6->stHeader.stIP.uiVersion = htons(ETHER_TYPE_IPV6);
		pstPacketV6->stHeader.stIP.uiHeaderLength = 0x05; // 20 Bytes
		pstPacketV6->stHeader.stTcp.uiDataOffset = 0x05;  // 20 Bytes
		pstPacketV6->stHeader.stIP.uiTotolLength = htons(nIpHeader + nTcpHeader + nReaderHeader + uiOriginalSize);
		//pstPacketV6->stHeader.stIP.uiTotolLength = htons(20 + 20 + uiOriginalSize);

		if (fSend)
		{
			// IPv6 Address
			//pstPacketV6->stHeader.stIP.uiSource = htonl((u_long)m_stHostAddress.sin_addr.s_addr);
			//pstPacketV6->stHeader.stIP.uiDestination = htonl((u_long)m_stLocalAddress.sin_addr.s_addr);;

			// TCP Port
			pstPacketV6->stHeader.stTcp.uiSourcePort = htons((u_short)m_stHostAddress.sin_port);
			pstPacketV6->stHeader.stTcp.uiDestinationPort = htons((u_short)m_stLocalAddress.sin_port);
		}
		else
		{
			// IPv6 Address
		...
		pstPacketV6->stHeader.stReader.uiType = htonl((u_long)uiPacketType);
		pstPacketV6->stHeader.stReader.uiSerialNo = htonl((u_long)0);
		pstPacketV6->stHeader.stReader.uiLength = htons((u_short)uiOriginalSize);
		//pstPacketV6->stHeader.stReader.ulDeviceID = htonl((u_long)m_ulDeviceID);
		pstPacketV6->stHeader.stReader.uiDeviceID = htonl((u_int64)m_uiDeviceID);
	}
	if (nGenerateSize < uiBufferSize)
	{
		lpbyBuffer[nGenerateSize] = 0x00; // NULL-String
	}
	return nGenerateSize;
}
...
int RfidInterface::Receive(unsigned int &uiPacketType, void* lpBuf, int nBufLen, int nFlags)
{
	...
	int nGenerateSize = GeneratePacket((unsigned char *)szBuffer, _countof(szBuffer), uiPacketType, (unsigned char *)lpBuf, nRecv, false);
...
int RfidInterface::Send(unsigned int uiPacketType, const void* lpBuf, int nBufLen, int nFlags)
{
	...
	int nGenerateSize = GeneratePacket((unsigned char *)szBuffer, _countof(szBuffer), uiPacketType, (unsigned char *)lpBuf, nBufLen, true);
	OnHookCallback(szBuffer, nGenerateSize);
...
int RfidInterface::Receive(void* lpBuf, int nBufLen, int nFlags)
{
	...
	{
		// Make TCP herader (like)
		PRFID_SNIFFER_PACKET_V4 pstPacketV4 = (PRFID_SNIFFER_PACKET_V4)szBuffer;

		nHookSize = sizeof(ETHERNET_HEADER);				// 14 Bytes
		nHookSize = sizeof(INTERNET_PROTOCOL_VERSION_4);	// 20 Bytes
		nHookSize = sizeof(TRANSMISSION_CONTROL_PROTOCOL);	// 20+N Bytes
		nHookSize = sizeof(RFID_TCP_V4_HEADER);
		nHookSize += nRecv;
		memcpy_s(pstPacketV4->byRaw, _countof(pstPacketV4->byRaw), lpBuf, nRecv);
		memset(&pstPacketV4->stHeader, 0, sizeof(pstPacketV4->stHeader));
		pstPacketV4->stHeader.stIP.uiVersion = htons(ETHER_TYPE_IPV4);
		pstPacketV4->stHeader.stIP.uiHeaderLength = 0x05; // 20 Bytes
		pstPacketV4->stHeader.stTcp.uiDataOffset = 0x05;  // 20 Bytes
		pstPacketV4->stHeader.stIP.uiTotolLength = htons(20 + 20 + nRecv);
		...
		// TCP Port
		//pstPacketV4->stHeader.stTcp.uiSourcePort = (u_short)m_stHostAddress.sin_port;
		pstPacketV4->stHeader.stTcp.uiSourcePort = htons((u_short)m_stHostAddress.sin_port);
		pstPacketV4->stHeader.stTcp.uiDestinationPort = htons((u_short)m_stLocalAddress.sin_port);
	...
	{
		// Make TCP herader (like)
		PRFID_SNIFFER_PACKET_V6 pstPacketV6 = (PRFID_SNIFFER_PACKET_V6)szBuffer;
		nHookSize = sizeof(RFID_TCP_V6_HEADER);
		nHookSize += nRecv;
		memcpy_s(pstPacketV6->byRaw, _countof(pstPacketV6->byRaw), lpBuf, nRecv);
		memset(&pstPacketV6->stHeader, 0, sizeof(pstPacketV6->stHeader));
		pstPacketV6->stHeader.stIP.uiVersion = htons(ETHER_TYPE_IPV6);
		pstPacketV6->stHeader.stIP.uiHeaderLength = 0x05; // 20 Bytes
		pstPacketV6->stHeader.stTcp.uiDataOffset = 0x05;  // 20 Bytes
		pstPacketV6->stHeader.stIP.uiTotolLength = htons(20 + 20 + nRecv);
	...
	OnHookCallback(szBuffer, nHookSize);
...
int RfidInterface::Send(const void* lpBuf, int nBufLen, int nFlags)
{
	...
	if (m_fIPv4)
	{
		...
		nHookSize = sizeof(RFID_TCP_V4_HEADER);
		nHookSize += nBufLen;
		memcpy_s(pstPacketV4->byRaw, _countof(pstPacketV4->byRaw), lpBuf, nBufLen);
		memset(&pstPacketV4->stHeader, 0, sizeof(pstPacketV4->stHeader));
		pstPacketV4->stHeader.stIP.uiVersion = htons(ETHER_TYPE_IPV4);
		pstPacketV4->stHeader.stIP.uiHeaderLength = 0x05; // 20 Bytes
		pstPacketV4->stHeader.stTcp.uiDataOffset = 0x05;  // 20 Bytes
		pstPacketV4->stHeader.stIP.uiTotolLength = htons(20 + 20 + nBufLen);
		...
		//	int nPacketSize = ntohs(pstSnifferPacket->stHeader.uiLength);
		//int nRawDataSize = ntohs(pstSnifferPacket->stHeader.uiLength);
	}
	else
	{
		// Make TCP herader (like)
		...
		nHookSize = sizeof(RFID_TCP_V6_HEADER);
		nHookSize += nBufLen;
		memcpy_s(pstPacketV6->byRaw, _countof(pstPacketV6->byRaw), lpBuf, nBufLen);
		memset(&pstPacketV6->stHeader, 0, sizeof(pstPacketV6->stHeader));
		pstPacketV6->stHeader.stIP.uiVersion = htons(ETHER_TYPE_IPV6);
...
bool RfidInterface::SetLinger(int nSocket)
{
	...
	if (setsockopt(nSocket, SOL_SOCKET, SO_LINGER, (char*)&oLinger, sizeof(oLinger)) == SOCKET_ERROR)
	{
...
bool RfidInterface::SetNameResolution(int nSocket, int nPort)
{
	...
	// 程式裡必須要有 message loop 來處理。但是，對於 NT Service program 而言，通常是沒有
	// message loop 的！也因此這限制了 asynchronous name resolution 的使用。底下是 blocking
...
bool RfidInterface::Create(UINT nSocketPort, int nSocketType, LPCTSTR lpszSocketAddress)
{
	...
	//int nSocketType = SOCK_STREAM; // SOCK_STREAM or SOCK_DGRAM.
    // A pointer to a string containing the network address of the connected socket, a dotted number such as "128.56.22.8". Passing the NULL string for this parameter indicates the CSocket instance should listen for client activity on all network interfaces.
...
#ifdef _UNICODE
		CT2A strTemp(lpszHostAddress);  // uses LPCTSTR conversion operator for CString and CT2A constructor
...
#else
		int nResult = inet_pton(AF_INET, lpszAscii, &m_stHostAddress.sin_addr.s_addr); // IPv4
#endif
...
bool RfidInterface::IsConnected()
{
	if (m_objSocket.m_hSocket)
	{
		...
		void* lpOptionValue = NULL; // A pointer to the buffer in which the value for the requested option is supplied.
		int nOptionLen = 0; // The size of the lpOptionValue buffer in bytes.
		int nLevel = SOL_SOCKET; // The level at which the option is defined; the only supported levels are SOL_SOCKET and IPPROTO_TCP.
		// return (SOCKET_ERROR != setsockopt(m_hSocket, nLevel, nOptionName, (LPCSTR)lpOptionValue, nOptionLen));
		//if (m_objSocket.GetSockOpt(nOptionName, lpOptionValue, &nOptionLen, nLevel))
		{
...
bool RfidInterface::StringToSockAddr(LPCTSTR lpszHostAddress, UINT nHostPort, SOCKADDR_IN &stSockAddr)
{
	...
	sockAddr.sin_addr.s_addr = inet_addr(lpszAscii);
#else
	int nResult = inet_pton(AF_INET, lpszAscii, &stSockAddr.sin_addr.s_addr); // IPv4
#endif
...
bool RfidInterface::SetRegulation(RFID_REGULATION emRegulation)
{
	...
	// Method 1
	//sprintf_s(szBuffer, _countof(szBuffer), "\n%s,%02X\r", "@N1", nPower); // 0x0A [CMD] 0x0D
	// Method 2
	sprintf_s(szSend, _countof(szSend), "\n%s,%02d\r", "N5", emRegulation); // 0x0A [CMD] 0x0D
	//int nSize = strlen(szSend);
...
bool RfidInterface::GetRegulation(RFID_REGULATION &emRegulation)
{
	...
	// Method 1
	//sprintf_s(szBuffer, _countof(szBuffer), "\n%s,%02X\r", "@N4", nPower); // 0x0A [CMD] 0x0D
	// Method 2
	sprintf_s(szSend, _countof(szSend), "\n%s,%02d\r", "N4", emRegulation); // 0x0A [CMD] 0x0D
	//int nSize = strlen(szSend);
...
bool RfidInterface::GetPower(int &nPower)
{
	...
	// Method 1
	//sprintf_s(szBuffer, _countof(szBuffer), "\n%s,%02X\r", "@N0", nPower); // 0x0A [CMD] 0x0D
	// Method 2
	sprintf_s(szSend, _countof(szSend), "\n%s,%02X\r", "N0", nPower); // 0x0A [CMD] 0x0D
	//int nSize = strlen(szSend);
...
bool RfidInterface::SetSingleAntenna(unsigned int uiAntenna, bool fHub, int * pnResult)
{
	...
	if (fHub == true)
		//sprintf_s(szBuffer, _countof(szBuffer), "\n%s,%02d\r", "@Antenna", nAntenna); // 0x0A [CMD] 0x0D
		sprintf_s(szSend, _countof(szSend), "\n%s%02d\r", "@Antenna", uiAntenna); // 0x0A [CMD] 0x0D
	else
		//sprintf_s(szBuffer, _countof(szBuffer), "\n%s,%d\r", "@Antenna", nAntenna); // 0x0A [CMD] 0x0D
		sprintf_s(szSend, _countof(szSend), "\n%s%d\r", "@Antenna", uiAntenna); // 0x0A [CMD] 0x0D
	//int nSize = strlen(szSend);
	...
	if (nRecv > 0)
	{
		...
		szReceive[nRecv] = 0; // Set null-string
		if (ParseSetSingleAntenna(szReceive, nRecv, &uiReturnAntenna, &fReturnHub))
		{
...
bool RfidInterface::SetLoopAntenna(unsigned int uiAntennas)
{
	...
	// Recv: <LF>@LoopAntennea01000100<CR><LF> <== 0x0A 0x40 0x4C 0x6F 0x6F 0x70 0x41 0x6e 0x74 0x65 0x6E 0x6E 0x61 0x30 0x31 0x30 0x30 0x30 0x31 0x30 0x30 0x30 0x0d 0x0a

	// Ex.
	//nAntennas = RF_ANTENNA_01 + RF_ANTENNA_02 + RF_ANTENNA_11 + RF_ANTENNA_12 + RF_ANTENNA_16 +
	//	RF_ANTENNA_17 + RF_ANTENNA_21 + RF_ANTENNA_22 + RF_ANTENNA_25 + RF_ANTENNA_31 + RF_ANTENNA_32;

	char szSend[MAX_SEND_BUFFER];
	char szReceive[MAX_RECV_BUFFER];
	unsigned int uiRecvCommand = 0; // The received command
	bool fResult = false;
	sprintf_s(szSend, _countof(szSend), "\n%s%08X\r", "@LoopAntenna", uiAntennas); // 0x0A [CMD] 0x0D
...
bool RfidInterface::GetLoopAntenna(unsigned int &uiAntennas)
{
	...
	//
	// Send: <LF>@LoopAntenna<CR>  <== 0x0A 0x40 0x4C 0x6F 0x6F 0x70 0x41 0x6e 0x74 0x65 0x6E 0x6E 0x61 0x0D
	// Recv: <LF>@LoopAntennaC1318C03<CR><LF> <== 0x0A 0x40 0x4C 0x6F 0x6F 0x70 0x41 0x6e 0x74 0x65 0x6E 0x6E 0x61
	//						 C1318C03             0x43 0x31 0x33 0x31 0x38 0x43 0x30 0x33 0x0D 0x0A
	// Recv: <LF>@LoopAntennaC1318C03<CR><LF> <== 0x0A 0x40 0x4C 0x6F 0x6F 0x70 0x41 0x6e 0x74 0x65 0x6E 0x6E 0x61
...
bool RfidInterface::SetLoopTime(unsigned int uiMilliseconds)
{
	...
	unsigned int uiRecvCommand = 0; // The received command
	bool fResult = false;
	sprintf_s(szSend, _countof(szSend), "\n%s%d\r", "@LoopTime", uiMilliseconds); // 0x0A [CMD] 0x0D
...
bool RfidInterface::SetSystemTime()
{
	...
	//                     ^^ Bug?
	// Send: <LF>@SETDATE20201109202806<CR>  <== 0x0A 0x40 0x53 045 054 044 041 054 045 0x32 0x30 0x32 0x30 0x31 0x31 0x31 0x30 0x39 0x32 0x30 0x32 0x38 0x30 0x36 0x0D
	// Recv: <LF>@2020/11/09 20:28:06.000<CR><LF> <== 0x0A 0x40 0x32 0x30 0x32 0x30 0x2F 0x31 0x31 0x2F
	//                                                0x30 0x39 0x20 0x32 0x30 0x3A 0x32 0x38 0x3A 0x30 0x36 0x2E 0x30 0x30 0x30 0x0D 0x0A
...
bool RfidInterface::SetTime(struct tm stTime)
{
	...
	//                     ^^ Bug?
	// Send: <LF>@SETDATE20201109202806<CR>  <== 0x0A 0x40 0x53 0x45 0x54 0x44 0x41 0x54 0x45 0x32 0x30 0x32 0x30 0x31 0x31 0x31 0x30 0x39 0x32 0x30 0x32 0x38 0x30 0x36 0x0D
	// Recv: <LF>@2020/11/09 20:28:06.000<CR><LF> <== 0x0A 0x40 0x32 0x30 0x32 0x30 0x2F 0x31 0x31 0x2F
	//                                                0x30 0x39 0x20 0x32 0x30 0x3A 0x32 0x38 0x3A 0x30 0x36 0x2E 0x30 0x30 0x30 0x0D 0x0A
...
bool RfidInterface::GetTime(struct tm &stTime)
{
	...
	//                                                0x30 0x39 0x20 0x32 0x30 0x3A 0x34 0x36 0x3A 0x35 0x37 0x2E 0x33 0x37 0x34 0x0D 0x0A
...
bool RfidInterface::ReadEPC(unsigned int uiStartAddress, unsigned int uiWordLength, RFID_TAG_DATA &stTagData, unsigned int * puiErrorCode)
{
	...
	sprintf_s(szSend, _countof(szSend), "\n%c%d,%d,%d\r", CMD_RFID_READ_BANK, RFID_MB_EPC, uiStartAddress, uiWordLength); // 0x0A [CMD] 0x0D
...
bool RfidInterface::ReadTID(unsigned int uiStartAddress, unsigned int uiWordLength, RFID_TAG_DATA &stTagData, unsigned int * puiErrorCode)
{
	...
	sprintf_s(szSend, _countof(szSend), "\n%c%d,%d,%d\r", CMD_RFID_READ_BANK, RFID_MB_TID, uiStartAddress, uiWordLength); // 0x0A [CMD] 0x0D
...
bool RfidInterface::ReadUserData(unsigned int uiStartAddress, unsigned int uiWordLength, RFID_TAG_DATA &stTagData, unsigned int * puiErrorCode)
{
	...
	sprintf_s(szSend, _countof(szSend), "\n%c%d,%d,%d\r", CMD_RFID_READ_BANK, RFID_MB_USER, uiStartAddress, uiWordLength); // 0x0A [CMD] 0x0D
...
bool RfidInterface::ReadEPCandTID(unsigned int uiStartAddress, unsigned int uiWordLength, RFID_TAG_DATA &stTagData, RFID_TAG_EPC *pstTagEPC, unsigned int * puiErrorCode)
{
	...
	//	0040   31 30 20 31 38 3a 33 31 3a 31 33 2e 31 38 39 2d   10 18:31 : 13.189 -
	//	0050   41 6e 74 65 6e 6e 61 32 2d 51 33 30 30 30 45 32   Antenna2 - Q3000E2
	//	0060   38 30 31 31 36 30 36 30 30 30 30 32 30 44 36 38   8011606000020D68
	//	0070   34 32 43 41 42 46 42 46 41 46 2c 52 45 32 38 30   42CABFBFAF, RE280
	//	0080   31 31 36 30 32 30 30 30 36 32 42 46 30 38 35 39   1160200062BF0859
	//	0090   30 39 41 44 0d 0a                                 09AD..
	...
	sprintf_s(szSend, _countof(szSend), "\n%c,%c%d,%d,%d\r", CMD_RFID_READ_EPC, CMD_RFID_READ_BANK, RFID_MB_TID, uiStartAddress, uiWordLength); // 0x0A [CMD] 0x0D
...
bool RfidInterface::ReadEPCandUserData(unsigned int uiStartAddress, unsigned int uiWordLength, RFID_TAG_DATA &stTagData, RFID_TAG_EPC *pstTagEPC, unsigned int * puiErrorCode)
{
	...
	//	0040   31 30 20 31 38 3a 33 31 3a 32 35 2e 30 32 31 2d   10 18:31 : 25.021 -
	//	0050   41 6e 74 65 6e 6e 61 32 2d 51 2c 52 0d 0a         Antenna2 - Q, R..
	...
	sprintf_s(szSend, _countof(szSend), "\n%c,%c%d,%d,%d\r", CMD_RFID_READ_EPC, CMD_RFID_READ_BANK, RFID_MB_USER, uiStartAddress, uiWordLength); // 0x0A [CMD] 0x0D
...
bool RfidInterface::ReadSingleTagEPC(bool fLoop)
{
	...
	// Recv: <LF>@2020/11/10 16:26:39.338-Antenna2-Q[]<CR><LF> <== 0x0A 0x40 0x32 30 32 30 2f 31 31 2f
	//                                                   31 30 20 31 36 3a 32 36 3a 33 39 2e 33 33 38 2d
	//                                                   41 6e 74 65 6e 6e 61 32 2d 52 0d 0a
	...
	//---------------------------------------------------------------------------
	//	0000   00 24 77 56 4f bc 04 92 26 58 dc 63 08 00 45 00.$wVO...&X.c..E.
	//	0010   00 2c 55 80 00 00 80 06 00 00 c0 a8 ec 1c c0 a8   ., U.............
	//	0020   ec 5b c8 92 03 e9 65 67 61 f4 f6 82 c0 01 50 18   .[....ega.....P.
	//	0030   ff 70 b0 e4 00 00 0a 40 51 0d                     .p.....@Q.
	...
	//	0040   31 36 20 32 30 3a 33 36 3a 35 35 2e 36 31 37 2d   16 20:36 : 55.617 -
	//	0050   41 6e 74 65 6e 6e 61 31 2d 51 33 30 30 30 45 32   Antenna1 - Q3000E2
	//	0060   38 30 31 31 36 30 36 30 30 30 30 32 30 44 36 38   8011606000020D68
	//	0070   34 33 30 30 36 46 42 41 37 44 0d 0a               43006FBA7D..

	//EPC:	E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 55.617, IP : 192.168.236.91
	//EPC : E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 55.657, IP : 192.168.236.91
	//EPC : E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 55.783, IP : 192.168.236.91
	//EPC : E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 55.806, IP : 192.168.236.91
	//EPC : E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 55.863, IP : 192.168.236.91
	//EPC : E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 55.903, IP : 192.168.236.91
	//EPC : E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 55.960, IP : 192.168.236.91
	//EPC : E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 56.000, IP : 192.168.236.91
	//EPC : E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 56.040, IP : 192.168.236.91
	//EPC : E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 56.114, IP : 192.168.236.91
	//EPC : E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 56.153, IP : 192.168.236.91
	//EPC : E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 56.278, IP : 192.168.236.91
	//EPC : E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 56.352, IP : 192.168.236.91
	//EPC : E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 56.392, IP : 192.168.236.91
	//EPC : E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 56.518, IP : 192.168.236.91
	//EPC : E28011606000020D6843006F, Antenna : 1, Time : 2020 / 11 / 16 20 : 36 : 56.592, IP : 192.168.236.91
	...
	if (fLoop)
	{
		sprintf_s(szSend, _countof(szSend), "\n@%c\r", CMD_RFID_READ_EPC); // 0x0A [CMD] 0x0D
...
bool RfidInterface::ReadMultiTagEPC(int nSlot, bool fLoop)
{
	...
	//  Request
	//	0000   00 24 77 56 4f bc 04 92 26 58 dc 63 08 00 45 00.$wVO...&X.c..E.
	//	0010   00 2c 55 bf 00 00 80 06 00 00 c0 a8 ec 1c c0 a8   ., U.............
	//	0020   ec 5b cc 9c 03 e9 4e 83 cf c3 fc dc ba 01 50 18   .[....N.......P.
	//	0030   ff 70 51 95 00 00 0a 40 55 0d.pQ....@U.
	...
	//	0040   31 36 20 32 30 3a 35 31 3a 30 34 2e 30 36 33 2d   16 20:51 : 04.063 -
	//	0050   41 6e 74 65 6e 6e 61 31 2d 55 33 30 30 30 45 32   Antenna1 - U3000E2
	//	0060   38 30 31 31 36 30 36 30 30 30 30 32 30 44 36 38   8011606000020D68
	//	0070   34 33 30 30 36 46 42 41 37 44 0d 0a               43006FBA7D..
	//
	// Response 2
	...
	//	0040   31 36 20 32 30 3a 35 31 3a 30 34 2e 30 36 35 2d   16 20:51 : 04.065 -
	//	0050   41 6e 74 65 6e 6e 61 31 2d 55 33 30 30 30 45 32   Antenna1 - U3000E2
	//	0060   38 30 31 31 36 30 36 30 30 30 30 32 30 44 36 38   8011606000020D68
	//	0070   34 32 43 43 46 46 35 44 43 44 0d 0a               42CCFF5DCD..
	...
	//	0040   31 36 20 32 30 3a 35 31 3a 30 34 2e 30 39 33 2d   16 20:51 : 04.093 -
	//	0050   41 6e 74 65 6e 6e 61 31 2d 55 0d 0a               Antenna1 - U..

	//	0000   04 92 26 58 dc 63 00 24 77 56 4f bc 08 00 45 00   ..&X.c.$wVO...E.
	//	0010   00 2f 3e f7 00 00 ff 06 23 08 c0 a8 ec 5b c0 a8   . / >.....#....[..
	//	0020   ec 1c 03 e9 cc 9c fc dc c6 99 4e 83 cf c7 50 18   ..........N...P.
	//	0030   00 ef 05 2a 00 00 0a 40 45 4e 44 0d 0a            ...*...@END..
	...
	{
		if ((nSlot > 0) && (nSlot <= 9))
			sprintf_s(szSend, _countof(szSend), "\n@%c%d\r", CMD_RFID_READ_MULTI_EPC, nSlot); // 0x0A @[CMD] 0x0D
		else
...
bool RfidInterface::SelectMatchingTag(RFID_TAG_DATA &stTagData)
{
	...
	//                                                   31 30 20 31 36 3a 32 36 3a 33 39 2e 33 33 38 2d
	//                                                   41 6e 74 65 6e 6e 61 32 2d 54 0d 0a
	...
	sprintf_s(szSend, _countof(szSend), "\n%c\r", CMD_RFID_SELECT_MATCHING); // 0x0A [CMD] 0x0D
...
bool RfidInterface::SetSession(RFID_SESSION emSession, RFID_TARGET emTarget)
{
	...
	bool fResult = false;
	if (emSession > 0)
		sprintf_s(szSend, _countof(szSend), "\n%c%02d%d\r", CMD_RFID_SET_SESSION, emSession, emTarget); // 0x0A [CMD] 0x0D
	else
		sprintf_s(szSend, _countof(szSend), "\n%c\r", CMD_RFID_SET_SESSION); // 0x0A [CMD] 0x0D
...
bool RfidInterface::OpenHeartbeat(unsigned int uiMilliseconds)
{
	...
	unsigned int uiRecvCommand = 0; // The received command
	bool fResult = false;
	sprintf_s(szSend, _countof(szSend), "\n%s%d\r", "@HeartbeatTime", uiMilliseconds); // 0x0A [CMD] 0x0D
...
#endif
	if ((fRemoveLeadingWord == true) && (strBuffer.length() > 4) && (strBuffer.at(0) == _T('\n')) &&
		(strBuffer.at(1) == _T('@')) &&
		(strBuffer.at(nBufferLength - 2) == _T('\r')) && (strBuffer.at(nBufferLength - 1) == _T('\n')))
	{
		strContent = strBuffer.substr(2, strBuffer.length() - 4);
		return true;
	}
	else
	if ((strBuffer.length() > 3) && (strBuffer.at(0) == _T('\n')) &&
		(strBuffer.at(nBufferLength - 2) == _T('\r')) && (strBuffer.at(nBufferLength - 1) == _T('\n')))
	{
...
bool RfidInterface::ParseVersion(const void* lpBuffer, int nBufferLength, RFID_READER_VERSION &stVersion)
{
	...
	if (aryVersion.size() > 0)
	{
		if (aryVersion.at(0).at(0) != _T('V'))
		{
			TString strDate;
			TString strAntenna;
			TStringArray aryFirmware = m_objTokenizer.Token(aryVersion.at(0), _T('-'));
			if (aryFirmware.size() > 0)
			{
				...
				for (int nIndex = 0; nIndex < aryFirmware.size(); nIndex++)
				{
					if (aryFirmware.at(nIndex).at(0) == _T('V'))
					{
						...
						stVersion.strFirmware = aryFirmware.at(nIndex).substr(1, aryFirmware.at(nIndex).length() - 1);
						break;
					}
				}
	...
	if (strContent.at(0) != _T('S'))
	{
		...
		if (aryContent.size() > 0)
		{
			for (int nIndex = 0; nIndex < aryContent.size(); nIndex++)
			{
				...
				{
					strID = aryContent.at(nIndex).substr(1, aryContent.at(nIndex).length() - 1);
					fResult = true;
					break;
				}
			}
...
bool RfidInterface::ParseSetRegulation(const void* lpBuffer, int nBufferLength, int * pnResult)
{
	...
	if (strContent.at(0) != _T('N'))
	{
		...
		if (aryContent.size() > 0)
		{
			for (int nIndex = 0; nIndex < aryContent.size(); nIndex++)
			{
				//if ((aryContent.at(nIndex).at(0) == _T('N')) && (aryContent.at(nIndex).at(0) == _T('5')))
				if (aryContent.at(nIndex).at(0) == _T('N'))
				{
					TString strValue = aryContent.at(nIndex).substr(1, aryContent.at(nIndex).length() - 1);
					// Decimal
					//int nRegulation = _tcstol(strValue.c_str(), NULL, 10);
					RFID_REGULATION emRegulation = (RFID_REGULATION)_tcstol(strValue.c_str(), NULL, 10);
					fResult = true;
					break;
				}
			}
...
bool RfidInterface::ParseSetSingleAntenna(const void* lpBuffer, int nBufferLength, unsigned int * puiAntenna, bool * pfHub)
{
	...
	{
		TString strValue = strContent.substr(strKeyword.length(), strContent.length() - strKeyword.length());
...
bool RfidInterface::ParseDateTime(LPCTSTR lpszDataTime, struct tm &stDateTime)
{
	...
	{
		TStringArray aryDate = m_objTokenizer.Token(aryDateTime.at(0).c_str(), _T("/"));
		if (aryDate.size() == 3)
		{
		...
		TStringArray aryTime = m_objTokenizer.Token(aryDateTime.at(1).c_str(), _T(":"));
		if (aryTime.size() == 3)
		{
			stDateTime.tm_hour = _tcstol(aryTime.at(0).c_str(), NULL, 10);
			stDateTime.tm_min = _tcstol(aryTime.at(1).c_str(), NULL, 10);
			TStringArray arySecond = m_objTokenizer.Token(aryTime.at(2).c_str(), _T("."));
			if (arySecond.size() == 2)
			{
...
bool RfidInterface::ParseAntenna(LPCTSTR lpszAntenna, unsigned int &uiAntenna)
{
	...
	if ((strAntenna.compare(0, strKeyword.length(), strKeyword) == 0) && (strAntenna.length() > strKeyword.length()))
	{
		TString strValue = strAntenna.substr(strKeyword.length(), strAntenna.length() - strKeyword.length());
		// Decimal
...
bool RfidInterface::ParseReadSingleEPC(const void* lpBuffer, int nBufferLength, RFID_TAG_DATA &stTagData, RFID_TAG_EPC *pstTagEPC, unsigned int * puiErrorCode)
{
	...
	// Recv: <LF>@2020/11/16 20:36:55.617-Antenna1-Q[]<CR><LF> <== 0a 40 32 30 32 ... 41 37 44 0d 0a
	//     DateTime: 2020/11/10 17:29:27.544
	//     Antennea: 1
	//     Data: None
	// Recv: <LF>@2020/11/16 20:36:55.617-Antenna1-Q3000E28011606000020D6843006FBA7D<CR><LF>
	//     DateTime: 2020/11/10 17:29:27.544
	//     Antennea: 1
	...
	if (aryContent.size() == 3)
	{
		...
		if ((strData.at(0) == _T(CMD_RFID_READ_SINGLE_EPC)) && (strData.length() > 1))
		{
			//stTagData.strEPC = aryTag.at(nDataIndex);
			// Response EPC Content: PC(4 Bytes) + EPC + CRC16(4 Bytes)
			//stTagData.strData = aryTag.at(nDataIndex).substr(1, aryTag.at(nDataIndex).length() - 1);
			stTagData.strEPC = strData.substr(1 + LEN_EPC_PEOTOCOL, strData.length() - (1 + LEN_EPC_PEOTOCOL + LEN_EPC_CRC16));
			if (pstTagEPC)
			{
				TString strPC = strData.substr(1, LEN_EPC_PEOTOCOL);
				TString strCRC = strData.substr(strData.length() - LEN_EPC_CRC16, LEN_EPC_CRC16);
...
bool RfidInterface::ParseReadEPCandTID(const void* lpBuffer, int nBufferLength, RFID_TAG_DATA &stTagData, RFID_TAG_EPC *pstTagEPC, unsigned int * puiErrorCode)
{
	...
	//     Antennea: 1
	//     Data: None
	// Recv: <LF>@2020/11/10 17:29:27.544-Antennea1-Q3000E28011606000020D6843006FBA7D, RE28011602000606F086009AD<CR><LF>
	//     DateTime: 2020/11/10 17:29:27.544
	//     Antennea: 1
	...
	//	0020   ec 1c 03 e9 c2 ec 6f 11 26 54 7c c1 3b 73 50 18   ......o.&T | .; sP.
	//	0030   00 ef 0f a8 00 00 0a 40 32 30 32 30 2f 31 31 2f   .......@2020/11 /
	//	0040   31 31 20 31 33 3a 33 35 3a 34 36 2e 31 36 37 2d   11 13:35 : 46.167 -
	//	0050   41 6e 74 65 6e 6e 61 31 2d 51 33 30 30 30 45 32   Antenna1 - Q3000E2
	//	0060   38 30 31 31 36 30 36 30 30 30 30 32 30 44 36 38   8011606000020D68
	//	0070   34 33 30 30 36 46 42 41 37 44 2c 52 45 32 38 30   43006FBA7D, RE280
	//	0080   31 31 36 30 32 30 30 30 36 30 36 46 30 38 36 30   11602000606F0860
	//	0090   30 39 41 44 0d 0a                                 09AD..
	...
	if (aryContent.size() == 3)
	{
		...
		if ((strData.at(0) == _T(CMD_RFID_READ_EPC_WITH_TID)) && (strData.length() > 1))
		{
			...
			for (int nDataIndex = 0; nDataIndex < aryTag.size(); nDataIndex++)
			{
				if (aryTag.at(nDataIndex).at(0) == _T(CMD_RFID_READ_EPC))
				{
					...
					// Response EPC Content: PC(4 Bytes) + EPC + CRC16(4 Bytes)
					//stTagData.strData = aryTag.at(nDataIndex).substr(1, aryTag.at(nDataIndex).length() - 1);
					stTagData.strEPC = aryTag.at(nDataIndex).substr(1+ LEN_EPC_PEOTOCOL, aryTag.at(nDataIndex).length() - (1+ LEN_EPC_PEOTOCOL + LEN_EPC_CRC16));
					if (pstTagEPC)
					{
						TString strPC = aryTag.at(nDataIndex).substr(1, LEN_EPC_PEOTOCOL);
						TString strCRC = aryTag.at(nDataIndex).substr(aryTag.at(nDataIndex).length() - LEN_EPC_CRC16, LEN_EPC_CRC16);
						pstTagEPC->strRaw = aryTag.at(nDataIndex).substr(1, aryTag.at(nDataIndex).length() - 1);
						pstTagEPC->uiProtocolControl = _tcstol(strPC.c_str(), NULL, 16);
						pstTagEPC->strEPC = stTagData.strEPC;
						pstTagEPC->uiCRC16 = _tcstol(strCRC.c_str(), NULL, 16);
					}
				...
				else if (aryTag.at(nDataIndex).at(0) == _T(CMD_RFID_READ_TID))
				{
					...
					stTagData.strTID = aryTag.at(nDataIndex).substr(1, aryTag.at(nDataIndex).length() - 1);
					fResult |= true;
				}
				else
				{
	...
	if (aryContent.size() > 0)
	{
		...
		for (int nIndex = 0; nIndex < aryContent.size(); nIndex++)
		{
			if (aryContent.at(nIndex).at(0) == _T(CMD_RFID_READ_EPC_WITH_TID))
			{
				...
				{
					TString strData = aryContent.at(nIndex); // aryContent.at(nIndex).substr(1, aryContent.at(nIndex).length() - 1);
					TStringArray aryTag = m_objTokenizer.Token(strData, _T(','));
					for (int nDataIndex = 0; nDataIndex < aryTag.size(); nDataIndex++)
					{
						if (aryTag.at(nDataIndex).at(0) == _T(CMD_RFID_READ_EPC))
						{
							//stTagData.strEPC = aryTag.at(nDataIndex);
							stTagData.strEPC = aryTag.at(nDataIndex).substr(1, aryTag.at(nDataIndex).length() - 1);
							fResult |= true;
						}
						else if (aryTag.at(nDataIndex).at(0) == _T(CMD_RFID_READ_TID))
						{
...
bool RfidInterface::ParseReadEPCandUserData(const void* lpBuffer, int nBufferLength, RFID_TAG_DATA &stTagData, RFID_TAG_EPC *pstTagEPC, unsigned int * puiErrorCode)
{
	...
	//	0040   31 31 20 31 36 3a 31 39 3a 30 34 2e 30 34 35 2d   11 16:19 : 04.045 -
	//	0050   41 6e 74 65 6e 6e 61 31 2d 51 33 30 30 30 45 32   Antenna1 - Q3000E2
	//	0060   38 30 31 31 36 30 36 30 30 30 30 32 30 44 36 38   8011606000020D68
	//	0070   34 33 30 30 36 46 42 41 37 44 2c 33 0d 0a         43006FBA7D, 3..
		...
		if ((strData.at(0) == _T(CMD_RFID_READ_EPC_WITH_TID)) && (strData.length() > 1))
		{
			...
			for (int nDataIndex = 0; nDataIndex < aryTag.size(); nDataIndex++)
			{
				...
				else if (aryTag.at(nDataIndex).at(0) == _T(CMD_RFID_READ_BANK))
				{
					...
					stTagData.strUser = aryTag.at(nDataIndex).substr(1, aryTag.at(nDataIndex).length() - 1);
					fResult |= true;
				}
				else
				{
			...
			if (aryContent.at(nIndex).at(0) == _T(CMD_RFID_READ_EPC_WITH_TID))
			{
				...
				else if (aryContent.at(nIndex).length() > 1)
				{
					...
					for (int nDataIndex = 0; nDataIndex < aryTag.size(); nDataIndex++)
					{
						...
						else if (aryTag.at(nDataIndex).at(0) == _T(CMD_RFID_READ_USER_DATA))
						{
							stTagData.strUser = aryTag.at(nDataIndex).substr(1, aryTag.at(nDataIndex).length() - 1);
							fResult |= true;
						}
...
bool RfidInterface::ParseReadMultiEPC(const void* lpBuffer, int nBufferLength, RFID_TAG_DATA &stTagData, RFID_TAG_EPC *pstTagEPC, unsigned int * puiErrorCode)
{
	...
	if (aryContent.size() == 3)
	{
		...
		if ((strData.at(0) == _T(CMD_RFID_READ_MULTI_EPC)) && (strData.length() > 1))
		{
			stTagData.strEPC = strData.substr(1 + LEN_EPC_PEOTOCOL, strData.length() - (1 + LEN_EPC_PEOTOCOL + LEN_EPC_CRC16));
			if (pstTagEPC)
			{
...
bool RfidInterface::ParseInventory(const void* lpBuffer, int nBufferLength, RFID_TAG_DATA &stTagData, RFID_TAG_EPC *pstTagEPC, unsigned int * puiErrorCode)
{
	...
	// Recv: <LF>@2020/11/10 16:26:39.338-Antenna2-U3000E28011606000020D68425ACFD9D3, RE2801160200072CF084B09AD<CR><LF>
	//
	...
	// 0050   41 6e 74 65 6e 6e 61 31 2d 55 33 30 30 30 45 32   Antenna1 - U3000E2
	// 0060   38 30 31 31 36 30 36 30 30 30 30 32 30 44 36 38   8011606000020D68
	// 0070   34 32 35 41 43 46 44 39 44 33 2c 52 45 32 38 30   425ACFD9D3, RE280
	// 0080   31 31 36 30 32 30 30 30 37 32 43 46 30 38 34 42   1160200072CF084B
	// 0090   30 39 41 44 0d 0a                                 09AD..
	...
	// 0040   32 31 20 31 35 3a 35 34 3a 33 33 2e 33 36 35 2d   21 15:54 : 33.365 -
	// 0050   41 6e 74 65 6e 6e 61 31 2d 55 33 30 30 30 45 32   Antenna1 - U3000E2
	// 0060   38 30 31 31 36 30 36 30 30 30 30 32 30 44 36 38   8011606000020D68
	// 0070   34 33 30 30 36 46 42 41 37 44 2c 52 45 32 38 30   43006FBA7D, RE280
	// 0080   31 31 36 30 32 30 30 30 36 30 36 46 30 38 36 30   11602000606F0860
	...
	// 0050   41 6e 74 65 6e 6e 61 34 2d 55 0d 0a 0a 40 45 4e   Antenna4 - U...@EN
	// 0060   44 0d 0a                                          D..
...
bool RfidInterface::ParseSelectMatching(const void* lpBuffer, int nBufferLength, RFID_TAG_DATA &stTagData)
{
	...
	if (aryContent.size() == 3)
	{
		...
		if ((strData.at(0) == _T(CMD_RFID_SELECT_MATCHING)) && (strData.length() == 1))
		{
			fResult = true;
		}
	}
...
bool RfidInterface::ParseSession(const void* lpBuffer, int nBufferLength, RFID_TAG_DATA &stTagData)
{
	...
	if (aryContent.size() == 3)
	{
		...
		if ((strData.at(0) == _T(CMD_RFID_SET_SESSION)) && (strData.length() == 1))
		{
			fResult = true;
		}
	}
